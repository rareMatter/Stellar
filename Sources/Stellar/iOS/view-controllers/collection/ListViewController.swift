...
class ListViewController: NLViewController, UICollectionViewDelegate {
    ...
    private var defaultListLayoutConfiguration: UICollectionLayoutListConfiguration {
        ...
        config.leadingSwipeActionsConfigurationProvider = { [unowned self] indexPath in
            configuration.leadingSwipeActions(itemInSnapshot(with: indexPath))
        }
        config.trailingSwipeActionsConfigurationProvider = { [unowned self] indexPath in
            configuration.trailingSwipeActions(itemInSnapshot(with: indexPath))
        }
    ...
    private var configuration: ListViewControllerConfiguration
...
//        set { selectionsSubject?.value = newValue }
    ...
    init(_ sectionSubject: CurrentValueSubject<[ListSection], Never>,
        ...
        if let layout = layout {
            collectionView = Self.makeCollectionView(with: layout,
                                                     delegate: self,
                                                     dragAndDropDelegate: nil,
                                                     backgroundColor: backgroundColor)
        }
        else {
            collectionView = Self.makeCollectionView(with: defaultListLayout,
                                                     delegate: self,
                                                     dragAndDropDelegate: nil,
                                                     backgroundColor: backgroundColor)
        }
    ...
    // -- drag
    // MARK: TODO: Drag and drop needs a rewrite after switching from internal snapshot to NS snapshot and Stellar API changes. See SContentCell.
    /*
...
#warning("bug in section drag and drop")
    ...
    func collectionView(_ collectionView: UICollectionView, performDropWith coordinator: UICollectionViewDropCoordinator) {
        ...
        // reordering item(s) only
            ...
            configuration.didInsertItemIntoSection(sourceItem, destinationSection)
        }
    ...
    func subscribeToStateChanges() {
        ...
        selectionsSubject?
            ...
            .sink { [unowned self] newSelections in
                self.updateSelections(to: newSelections,
                                      withAnimation: !(viewIfLoaded?.isHidden ?? true))
            }
            .store(in: &cancellables)
    }
    
    static
    func makeCollectionView(with layout: UICollectionViewLayout,
                            delegate: UICollectionViewDelegate,
                            dragAndDropDelegate: (UICollectionViewDragDelegate & UICollectionViewDropDelegate)?,
                            backgroundColor: UIColor) -> UICollectionView {
        ...
        // -- drag and drop
        if let delegate = dragAndDropDelegate {
                ...
                switch responderNotifierState.status {
                    ...
                    case .resigned:
                        ...
                        if let nextResponderItem =
                            self.configuration.subsequentFirstResponder(currentResponderItem) {
...
extension ListViewController {
    ...
    func handleSelection(at indexPath: IndexPath, isContinuous continuous: Bool = false) {
        ...
        if mode == .editing {
            if continuous {
                ...
                if self.multiselectSessionData == nil {
                    multiselectSessionData = .init(isSelecting: !editingSelections.contains(item))
                }
        ...
        // if not editing, process as tap (normal mode)
            ...
            // TODO: This will be switched to canExpand/canCollapse properties located in the cell.
            /*
            ...
            if isSectionHeader {
                ...
                if isCollapsed {
                    if configuration.canExpand(item) {
                    ...
                    if configuration.canCollapse(item) {
...
extension ListViewController {
    ...
    func sectionInSnapshot(with indexPath: IndexPath) -> SectionType {
    ...
    func applyListMode(_ mode: ListMode) {
        switch mode {
            ...
            case .editing:
                ...
                updateSelections(to: editingSelections, withAnimation: true)
